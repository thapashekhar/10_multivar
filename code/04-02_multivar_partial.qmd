...
lm_dayl.apr <- lm(data = weather, strength_gtex~ mean_dayl.s_Apr)%>%
  tidy()%>%
...
```{r pca model}
mod_pca <- prcomp(weather_n, scale. = T)  #scale. = T  makes the pca do scaling or normalized by itself
mod_pca
summary(mod_pca)
...
ggplot(weather_postpca, aes(x = PC1, y= strength_gtex))+
  geom_point()+
  geom_smooth(method = "lm")
...
```{r kmeans model }
mod_km <- kmeans(weather_norm, centers = 6, nstart = 10)
...
# Total error x k
fviz_nbclust(weather_norm, method = "wss", k.max =10, FUNcluster = kmeans)
...
```{r mod_km4 }
mod_km4 <- kmeans(weather_norm, centers =4, nstart = 10)
...
```{r kmeans PCA plot}
fviz_cluster(mod_km4, data = weather_norm)
